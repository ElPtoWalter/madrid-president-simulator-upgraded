...
function bindStaticEvents(){
  ...
  els.btnPlan100?.addEventListener('click', aiBudgetPlan100);
  els.btnPlan100Dash?.addEventListener('click', aiBudgetPlan100);
  els.btnPlanRealistic?.addEventListener('click', aiRealisticPlan);
  els.btnPlanRealisticDash?.addEventListener('click', aiRealisticPlan);
  els.btnPlanClean?.addEventListener('click', aiCleanSquadPlan);
  els.btnPlanCleanDash?.addEventListener('click', aiCleanSquadPlan);
  els.btnMarketBargains?.addEventListener('click', () => { switchTab('market'); if(els.marketValue) els.marketValue.value=35; if(els.marketAge) els.marketAge.value=27; renderMarket(); toast('Mercado filtrado por oportunidades razonables.'); });
  els.btnMarketBargainsDash?.addEventListener('click', () => { switchTab('market'); if(els.marketValue) els.marketValue.value=35; if(els.marketAge) els.marketAge.value=27; renderMarket(); toast('Mercado filtrado por oportunidades razonables.'); });
...
function faceHtml(p){
  ...
  const src = p.remotePhoto || p.photo || '';
  if(src){
    return `<div class="face face-visible" data-face="${p.id}" data-player-id="${p.id}" data-wiki="${escapeAttr(wikiTitle)}"><img class="face-img" src="${escapeAttr(src)}" alt="${escapeAttr(p.name)}" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove();this.parentElement.textContent='${escapeAttr(initials)}';this.parentElement.classList.add('face-fallback')"></div>`;
  }
...
function setFaceImage(el, src, alt='', onError=null){
  ...
  img.onload = () => {
    ...
    if(current && current.src === img.src) return;
    el.textContent='';
    el.appendChild(img);
...
function renderExecutiveCommand(){
  const nodes=[...document.querySelectorAll('.executive-command-slot')];
  if(!nodes.length && els.executiveCommand) nodes.push(els.executiveCommand);
  if(!nodes.length) return;
  const f=computeFinance(), a=computeAnalysis(), r=computeRealityScore();
  const needs=detectNeedPositions();
  const active=activePlayers();
  const problems=[];
  if(needs.length) problems.push(`Faltan perfiles: ${needs.slice(0,4).join(', ')}`);
  if(f.balance < -180) problems.push(`Balance agresivo: ${fmtMoney(f.balance)}`);
  if(f.salaryDelta > 25) problems.push(`Masa salarial sube ${fmtMoney(f.salaryDelta)}`);
  if(active.length>28) problems.push(`Plantilla larga: ${active.length} jugadores activos`);
  if((a.overall||0)<8) problems.push('XI todavía por debajo de nivel Champions');
  const status = r.total>=84 ? 'Proyecto competitivo' : r.total>=70 ? 'Proyecto mejorable' : 'Necesita cirugía';
  const html = `<div class="command-grid">
    <div class="command-main"><small>Diagnóstico ejecutivo</small><b>${status}</b><span>Reality Score ${r.total}/100 · XI ${a.overall? a.overall.toFixed(1):'—'}/10 · Balance ${fmtMoney(f.balance)}</span></div>
    <div><small>Necesidades</small><b>${needs.length?needs[0]:'Cubiertas'}</b><span>${needs.length?needs.join(' · '):'La plantilla está compensada por líneas.'}</span></div>
    <div><small>Riesgo financiero</small><b>${f.risk}</b><span>Ventas ${fmtMoney(f.sales)} · Fichajes ${fmtMoney(f.buys)}</span></div>
    <div><small>Alertas</small><b>${problems.length}</b><span>${problems.slice(0,2).join(' · ') || 'Sin alertas críticas.'}</span></div>
  </div>`;
...
function opportunityScore(p){
  const needs=detectNeedPositions();
  let score=(p.rating||75)*1.4 + (p.potential||75)*.65 - (p.value||0)*.32 - (p.salary||0)*1.1;
  if(needs.some(n=>p.position===n||p.secondary?.includes(n))) score+=20;
  if((p.tags||[]).some(t=>['mou','físico','pivote','central zurdo','oportunidad','barato','LaLiga','español'].includes(t))) score+=8;
  if(p.age>=22 && p.age<=28) score+=6;
  if(['muy alta','intocable'].includes(p.difficulty)) score-=14;
  return Math.round(clamp(score,0,100));
}
function renderOpportunityBoard(){
  if(!els.opportunityBoard) return;
  const market=playersByStatus('market').filter(p=>!isSigned(p.id)).map(p=>({...p,opp:opportunityScore(p)})).sort((a,b)=>b.opp-a.opp).slice(0,12);
  els.opportunityBoard.innerHTML = market.map(p=>`<article class="op-card"><div>${faceHtml(p)}<div><b>${escapeHtml(p.name)}</b><small>${p.position} · ${escapeHtml(p.club)} · ${fmtMoney(estimateBuyCost(p))}</small></div></div><span class="pill">${p.opp}/100</span><p>${opportunityReason(p)}</p><div class="card-actions"><button class="btn btn-primary" data-action="negotiate" data-id="${p.id}">Negociar</button><button class="btn btn-soft" data-action="target" data-id="${p.id}">Radar</button></div></article>`).join('');
  bindCardEvents(els.opportunityBoard);
}
function opportunityReason(p){
  const needs=detectNeedPositions(); const reasons=[];
  if(needs.some(n=>p.position===n||p.secondary?.includes(n))) reasons.push('cubre una necesidad');
  if(p.value<=35) reasons.push('coste controlado');
  if(p.age<=23) reasons.push('potencial futuro');
  if((p.tags||[]).includes('físico')||(p.tags||[]).includes('pivote')) reasons.push('encaja con Mourinho');
  return reasons.length ? `Recomendado porque ${reasons.join(', ')}.` : 'Opción interesante por rendimiento/precio dentro del radar.';
}
function renderRumorIntelligence(){
  if(!els.rumorIntelligence) return;
  const rumors=(data.rumors||[]).slice().sort((a,b)=>(b.confidence||0)-(a.confidence||0)).slice(0,8);
  els.rumorIntelligence.innerHTML = rumors.map(r=>{const p=getPlayer(r.playerId); if(!p)return''; const lvl=(r.confidence||0)>=75?'hot':(r.confidence||0)>=55?'warm':'cold'; return `<div class="rumor-signal ${lvl}"><b>${escapeHtml(p.name)}</b><span>${escapeHtml(r.status)} · ${r.confidence||0}% · ${escapeHtml(r.source||'Radar')}</span><small>${escapeHtml(r.note||r.lastTitle||'')}</small></div>`}).join('') || empty('Sin señales todavía. Ejecuta AutoScout en Actions.');
}
function renderSquadAuditPro(){
  if(!els.squadAuditPro) return;
  const active=activePlayers(); const f=computeFinance();
  const avgAge=avg(active.map(p=>p.age)); const value=sum(active.map(p=>p.value));
  const starters=lineupPlayers(); const bench=benchPlayers();
  const rows=[['Jugadores activos',active.length, active.length<22?'bad':active.length>28?'warn':'good'],['Edad media',avgAge.toFixed(1), avgAge>28?'warn':avgAge<24?'warn':'good'],['Valor plantilla',fmtMoney(value),'good'],['Masa salarial',fmtMoney(f.salaryNow),f.salaryDelta>30?'bad':f.salaryDelta>15?'warn':'good'],['Titulares elegidos',starters.length,starters.length===11?'good':'warn'],['Banquillo útil 80+',bench.filter(p=>p.rating>=80).length,bench.filter(p=>p.rating>=80).length<7?'warn':'good']];
  els.squadAuditPro.innerHTML = `<table><tbody>${rows.map(([k,v,cls])=>`<tr class="${cls}"><td>${k}</td><td><b>${v}</b></td></tr>`).join('')}</tbody></table>`;
}
function renderScoutPipeline(){
  const nodes=[...document.querySelectorAll('.scout-pipeline-slot')];
  if(!nodes.length && els.scoutPipeline) nodes.push(els.scoutPipeline);
  if(!nodes.length) return;
  const autoPlayers=(data.players||[]).filter(p=>String(p.source||'').includes('AutoScout')).length;
  const remote=(data.players||[]).filter(p=>p.remotePhoto).length;
  const meta=data.meta?.autoMarket||{};
  const html = `<div class="pipeline"><div><b>1</b><span>Lee medios/RSS</span><small>${meta.articlesFetched||0} noticias último ciclo</small></div><div><b>2</b><span>Detecta nombres</span><small>${meta.detections||0} detecciones</small></div><div><b>3</b><span>Crea fichables</span><small>${autoPlayers} jugadores AutoScout</small></div><div><b>4</b><span>Hidrata fotos</span><small>${remote} fotos reales guardadas</small></div></div>`;
  nodes.forEach(n=>n.innerHTML=html);
}
function aiBudgetPlan100(){
  const needs=detectNeedPositions(); let budget=100, chosen=[];
  for(const need of (needs.length?needs:['MCD','DFC','ED'])){
    const option=playersByStatus('market').filter(p=>!isSigned(p.id)&&(p.position===need||p.secondary?.includes(need))).sort((a,b)=>opportunityScore(b)-opportunityScore(a))[0];
    if(option && estimateBuyCost(option)<=budget+20 && !chosen.includes(option)){ chosen.push(option); budget-=estimateBuyCost(option); }
  }
  chosen.forEach(p=>{ if(!state.targets.includes(p.id)) state.targets.push(p.id); });
  pushHistory('advice',`Plan 100M generado: ${chosen.map(p=>p.name).join(' · ') || 'sin objetivos claros'}`,0,{suggestions:chosen.map(p=>p.id)});
  saveAndRender(); switchTab('market'); toast('Plan 100M añadido al radar.');
}
function aiRealisticPlan(){
  const options=playersByStatus('market').filter(p=>!isSigned(p.id)&&!['muy alta','intocable'].includes(p.difficulty)&&estimateBuyCost(p)<=70).sort((a,b)=>opportunityScore(b)-opportunityScore(a)).slice(0,5);
  options.forEach(p=>{ if(!state.targets.includes(p.id)) state.targets.push(p.id); });
  pushHistory('advice',`Plan realista: ${options.map(p=>p.name).join(' · ')}`,0,{suggestions:options.map(p=>p.id)});
  saveAndRender(); switchTab('market'); toast('He marcado 5 operaciones realistas en radar.');
}
function aiCleanSquadPlan(){
  const surplus=benchPlayers().filter(p=>p.status==='squad' && p.rating<82 && p.value>=15).sort((a,b)=>b.value-a.value).slice(0,4);
  pushHistory('advice',`Limpieza sugerida: ${surplus.map(p=>`${p.name} (${fmtMoney(estimateSellOffer(p))})`).join(' · ') || 'sin ventas claras'}`,0,{suggestions:surplus.map(p=>p.id)});
  renderHistory(); toast('Plan de salidas añadido al historial.');
}
...
function fallbackData(){
  return {meta:{appName:'Madrid President Simulator 5.0',lastManualReview:'sin data.json'},players:[],rumors:[],formations:{'4-3-3':[]},projects:{champions:{name:'Proyecto Champions',description:''}},sources:[]};
}
